...
Ledger: **[`RESULTS.md`](RESULTS.md)** (generated — do not hand-edit the
table).

Same shape as llama.cpp’s [`llama-bench`](https://github.com/ggerganov/llama.cpp/tree/master/tools/llama-bench):
native CLI, `pp512` / `tg128`, compare against `llama-bench` on the same
GGUF. No HTTP, chat template, tokenizer, or sampler in the loop.
...
| Receipts | [`receipts/engine/`](receipts/engine/) |
...
**fresh child process**. That is required, not tidiness: backend
selection reads process-global environment and the rayon pool is built
once, so benchmarking several backends inside one process would silently
...
`{id}_{backend}.json`. `--render` rewrites the engine table in
`RESULTS.md` between HTML markers, leaving the Open notes alone.
...
quote a ratio without a receipt under [`receipts/engine/`](receipts/engine/).
...
Always record GPU/driver when quoting CUDA numbers. See
