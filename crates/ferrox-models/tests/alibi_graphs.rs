...
//! score in place of any rotation (`ferrox_core::alibi` is the slope
//! formula, `ferrox_models::alibi` the graphs and where each gets its
...
//!     crates/ferrox-models/tests/fixtures/<arch>_tiny.gguf --arch <arch> [--clamp] [--pos-embd]
...
use ferrox_models::alibi::{max_alibi_bias, ALIBI_ARCHITECTURES};
...
fn the_bias_and_the_absence_of_rotation_are_visible_in_the_logits() {
    ...
    let saved = d
        .alibi_slopes
        .replace(ferrox_core::alibi::slopes(4, 4.0).unwrap());
    let worst = worst_vs(&decode(&d), &REFACT_GOLDEN);
