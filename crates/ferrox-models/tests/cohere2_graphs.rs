...
//! | `cohere2_nowindow` | the REQUIRED window key left out; libllama refuses the file, and so does ferrox |
//!
...
//!     crates/ferrox-models/tests/fixtures/cohere2_tiny.gguf
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_cohere2_fixture.py \
//!     crates/ferrox-models/tests/fixtures/cohere2_pattern_tiny.gguf --pattern-key
...
/// so does ferrox, by name, rather than run it with no layer rotated.
#[test]
fn a_file_without_the_window_key_is_refused() {
    let file = ferrox_gguf::GgufFile::open(graph_fixture_path(NOWINDOW)).expect("fixture opens");
    assert_eq!(
        ferrox_models::swa_geometry::window_required(COHERE2),
        Some("cohere2.cpp:13")
    );
