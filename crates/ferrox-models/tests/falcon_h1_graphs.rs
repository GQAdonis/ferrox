...
//!     crates/ferrox-models/tests/fixtures/falcon_h1_tiny.gguf [--no-ssm-norm | --output]
//! /tmp/ref_logits crates/ferrox-models/tests/fixtures/falcon_h1_tiny.gguf 3 7 11 19 23 5
//! ```
...
fn both_parallel_branches_reach_the_residual() {
    ...
    let zero = |rows: usize, cols: usize| {
        ferrox_core::weight_matrix::WeightMatrix::F32(ferrox_core::Tensor::new(
            vec![0.0; rows * cols],
            vec![rows, cols],
        ))
    };
