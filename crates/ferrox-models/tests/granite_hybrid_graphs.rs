...
//! `layer_shapes::AttnShape::Mamba2` / `ferrox_core::recurrent_state`.
//!
...
//!     crates/ferrox-models/tests/fixtures/granitehybrid_tiny.gguf [--rope | --moe]
//! /tmp/ref_logits crates/ferrox-models/tests/fixtures/granitehybrid_tiny.gguf 3 7 11 19 23 5
//! ```
...
fn paged_decode_and_paged_prefill_match_contiguous() {
    ...
    let mut paged2: Vec<ferrox_core::cache::PagedKvCache> = (0..d.config.n_layers)
        .map(|_| ferrox_core::cache::PagedKvCache::new())
        .collect();
    let logits = d
