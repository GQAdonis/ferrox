...
//!     crates/ferrox-models/tests/fixtures/llama4_tiny.gguf [--128e --output | --noswa | --dense]
//! /tmp/ref_logits crates/ferrox-models/tests/fixtures/llama4_tiny.gguf 3 7 11 19 23 5
//! REF_N_CTX=8256 /tmp/ref_logits crates/ferrox-models/tests/fixtures/llama4_tiny.gguf $(long_prompt)
//! ```
//!
...
use ferrox_models::config::{BatchWindow, ModelConfig, RopeLayout};
...
fn every_llama4_seam_is_visible_in_the_logits() {
    ...
    d.config.attn_temperature = Some(ferrox_models::attn_temperature::AttnTemperature {
...
fn the_two_shapes_llama_cpp_cannot_run_are_refused_by_name() {
    for (name, needle) in [
        ...
        let file = ferrox_gguf::GgufFile::open(graph_fixture_path(name)).expect("opens");
