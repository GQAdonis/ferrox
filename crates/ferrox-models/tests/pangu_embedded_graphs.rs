...
//! `TextModel` with an `lm_head`; "Embedded" means edge devices). ferrox
...
//!     crates/ferrox-models/tests/fixtures/pangu_embedded_tiny.gguf [--fused-qkv | --output]
//! /tmp/ref_logits crates/ferrox-models/tests/fixtures/pangu_embedded_tiny.gguf 3 7 11 19 23 5
//! ```
...
fn the_loaded_decoder_is_the_graph() {
    ...
    assert!(!ferrox_models::embedding_model::is_embedding_arch(
        "pangu-embedded"
    ));
