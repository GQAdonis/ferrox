...
//! residual (`ferrox_models::parallel_residual`) over the biased
//! LayerNorm, Q/K/V biases, the REQUIRED `attn_output.bias` / `ffn_up.
//! bias` / `ffn_down.bias` with the ungated GELU (`ferrox_models::
...
//!     crates/ferrox-models/tests/fixtures/phi2_tiny.gguf
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_phi2_fixture.py \
//!     crates/ferrox-models/tests/fixtures/phi2_fused_tiny.gguf --fused
...
use ferrox_models::proj_bias::{Presence, OUTPUT_BIAS_CREATORS};
...
fn report_kl_against_llama_cpp() {
    ...
    println!(
        "{PHI2}: KL(llama.cpp || ferrox) = {:.3e}, max |delta| = {:.3e}",
