...
/// Every flag `ferrox-server` accepts.
///
/// Deliberately the *only* place the server's command line is defined:
/// `ferrox-server` parses it via [`ServerArgs::parse_llama_style`] and
/// ferrox-cli's `serve` subcommand embeds this same struct, so a flag
/// added here reaches both front ends and neither can quietly ship a
/// subset of the other.
///
/// No `name` in the `command` attribute on purpose -- clap defaults it
/// to the package name (`ferrox-server`) for the binary, and leaves the
/// subcommand's own name (`serve`) alone when this is embedded.
...
impl ServerArgs {
    ...
    /// express, which are rewritten to their long forms first.
    ///
    /// Exits the process on a parse error or on `--help`, exactly as
    /// `clap::Parser::parse_from` does.
    pub fn parse_llama_style<I, T>(argv: I) -> Self
    where
        I: IntoIterator<Item = T>,
        T: Into<String>,
    {
        Self::parse_from(rewrite_llama_style_argv(
...
/// Runs the server to completion: applies the CLI's environment
/// overrides, claims the single-instance registry slot, builds the Tokio
/// runtime and blocks on the async serving loop.
///
/// This is the whole public entry point. `ferrox-server`'s own `main` is
/// a two-line shim over it, and `ferrox serve` (ferrox-cli's optional
/// `serve` feature) calls it with the same [`ServerArgs`], so the two
/// front ends cannot drift apart in flags or in behaviour.
///
/// Blocking, and it builds its own multi-threaded runtime -- do not call
/// it from inside one.
...
mod tests {
    ...
    /// The library entry point must parse argv exactly as the old
    /// in-`main` code did, llama.cpp short-option rewrite included --
    /// `ferrox-server`'s binary is now two lines that call this and
    /// nothing else, so a rewrite that stopped happening here would
    /// stop happening for the binary too.
    #[test]
    fn public_arg_parser_applies_the_llama_style_rewrite() {
        let args = ServerArgs::parse_llama_style([
            "ferrox-server",
            "-ngl",
            "7",
            "-dev",
        ...
        assert_eq!(args.n_gpu_layers, Some(GpuLayers::Count(7)));
        assert_eq!(args.device, Some(OffloadDevice::Cuda));
        assert_eq!(args.model.as_deref(), Some("model.gguf"));
    }

    /// clap's own consistency check over the one struct that defines
    /// the server's command line. It catches duplicate long options and
    /// duplicate short options at test time rather than at the first
    /// `--help` a user types.
    #[test]
