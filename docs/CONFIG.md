...
| `FERROX_CPU_POOL` | `rayon` (default) or `spin`. `spin` runs decode on a persistent pool parked on a spin-then-park barrier, the shape llama.cpp's `ggml_threadpool` uses, instead of forking and joining per operation. **Opt-in, and now measured on x86** (2026-09-04, dedicated 10-core Xeon E5-2630 v4, idle, `tg128`): `spin` is **+49% at 135M, +23% at 3B, +15% at 8B**. The gain shrinks as the model grows, which is the per-operation-constant signature the pool was built for, and it grows with thread count (+23% at 6 threads vs +49% at 10 on the same 135M file). **It is still opt-in because a spin barrier degrades under contention**: on a host with a runaway daemon holding a core, the same comparison read 31% SLOWER at 135M. A quiet aarch64 measurement is the missing one. Output is token-identical on both settings, which is tested |
