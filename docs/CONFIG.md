...
| `FERROX_CB_MAX_CONTEXT` | Continuous batching: token positions (prompt + `max_tokens`) any one request may ask for. Over it, `400` with `code: context_length_exceeded`, `estimated_bytes` and `limit_bytes`. Unset means no per-request ceiling |
