...
2. **Ferrox knows it and has not implemented it.** `llama4`,
   ...
   it, and `command-r` and `falcon` since); `cohere2`, `cohere2moe` and `phi2` still
...
6. **Nobody has ever verified this architecture against llama.cpp.**
   ...
   logit comparison against real `libllama`, or a fixture; **66** do
   ...
   `falcon`, `gemma`, `gemma2`, `gemma3`, `phi3`, `gpt-oss`, `dots1`).
