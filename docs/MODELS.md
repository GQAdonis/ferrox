...
| A recurrent block at the attention site (`head_count_kv 0`) | CLOSED for the short convolution (`ferrox_models::shortconv`, `layer_shapes::AttnShape::ShortConv`, the state as the layer's KV history on all three backings): `lfm2` and `lfm2moe` run on it. CLOSED for the Mamba-2 block (`ferrox_models::mamba2`, `ferrox_core::mamba2`, `AttnShape::Mamba2`, the state as `ferrox_core::recurrent_state::RecurrentState` beside the cache): `granitehybrid` runs on it. `nemotron-h` (one block per layer, no FFN on a Mamba layer), `falcon-h1` (attention and Mamba-2 in parallel), `jamba` and `plamo2` (Mamba-1) name what they need in `layer_shapes::ZeroKvLayer` and refuse |
