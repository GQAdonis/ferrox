...
| Phi-4-mini-Instruct Q4_K_M | New PhiFamily pin; Metal ~parity |
| Llama-3.1-8B-Instruct Q4_K_M | Mainstream dense; Metal fair-chat **ahead** (~0.92×) |
| Llama-3.2-3B-Instruct Q4_K_M | Dense; Metal ~parity (~0.97×) |
...
|---|---|
| TinyLlama-1.1B-Chat-v1.0 Q8_0 | Metal ~0.96×; CPU pin exists |
| Llama-3.2-1B / 3B Q4_K_M | Metal ~1.00× / **~0.97×** (concurrent FFN encode) |
...
| OLMoE-1B-7B-0924 Q4_0 | Metal **~1.59×** (concurrent `matvec_id` gate∥up); CPU **~1.40×** (shared Q8 act across top-k) — still trails llama; next: expert residency hoist |
...
| Phi-3-mini-4k-Instruct Q4 | Metal ~0.92×; fused QKV; FA-vec d=96 |
| Phi-4-mini-Instruct Q4_K_M | Metal ~1.00×; CLI ~1.04× — [`phi4_mini_q4km_metal`](../benchmarks/receipts/pins/phi4_mini_q4km_metal.json) |
...
| Backend | Status |
|---|---|
| CPU | Dense + MoE; `FERROX_CPU_INT_DOT=1` on suite (shared Q8 act across MoE top-k) |
| Metal | Dense + attn + MoE; concurrent encode (gate∥up, Q∥K∥V) + buffer barriers; FA-vec decode d=64/96/128/256; FA-vec prefill d=128/256 + softcap |
