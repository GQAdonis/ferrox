...
- **Metal MoE** — expert residency + residual fuse; **llama-style** one Concurrent CB + `MoeMemRanges` barriers (ported from `ggml_mem_ranges`). Next: closer Q4_0 `mul_mv_id` / `mul_mm_id` prefill; multi-CB encode overlap.
