...
| # | What | Blocked on |
|---|---|---|
| [#27](https://github.com/antonellof/ferrox/issues/27) | CPU decode is scheduling-bound. **Measured on x86 (2026-09-04)**: the persistent pool is +49% at 135M, +23% at 3B, +15% at 8B on a dedicated 10-core Xeon, and the gain shrinks with model size exactly as a per-operation constant should. Still opt-in behind `FERROX_CPU_POOL=spin` | A quiet **aarch64** measurement. The one aarch64 run available read 31% slower at 135M, but on a host with a daemon holding a core, and a spin barrier is the design that suffers most under contention |
