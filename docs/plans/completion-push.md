...
todos:
  - id: wave-0-unblock
    content: "WAVE 0, one agent, must land before wave 2 touches the paged path. Fix the paged-KV GPU correctness bug. Measured 2026-08-24: identical prompt, seed and model (Llama-3.2-3B Q4_K_M), CPU paged answers 'Blue and Red.', Metal contiguous answers 'Blue and Red.', Metal paged answers 'Blue ( question mark;>a> is a> is'. Paging alone is fine, Metal alone is fine, the two together are not. Narrowed already: forward_token_paged has NO Metal attention arm, so attention runs on the CPU while the QKV projection (apply_gpu_multi) and the dense FFN (launch_dense_ffn_swiglu_batch) still run on the GPU; the contiguous path references metal:: 32 times, the paged path once. Suspect K or V reaching the page store in a different state (pre/post RoPE, or a dtype/layout the CPU attention does not expect) than the CPU attention reads back. DONE MEANS: `ferrox verify --backend metal --prompt-tokens 64` identical on a dense model, OLMoE and a sliding-window model, run several times because a mixed-path bug is a race, AND the refusal in ferrox-server/src/lib.rs removed in the same commit. Closes freetoken's `paged-decode-path` and unblocks `wire-radix-prefix-cache`, `radix-on-the-batcher`, `window-slide-during-decode`"
    status: pending
