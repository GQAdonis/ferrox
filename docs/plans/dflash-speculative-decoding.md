...
— and drafting latency is something a CPU/Metal engine can actually move.
...
3. `spec-drafter-trait` + `spec-cache-resume` + `spec-acceptance-metrics` —
   the engine-side foundation, all three independent of dFlash.
