...
todos:
  ...
  - id: ui-transfer-stats
    ...
    status: completed
  - id: ui-frontend-chat
    content: "Chat: streaming, markdown+code, reasoning blocks, sampling controls, model switch mid-conversation, conversation tree persisted SERVER-side (SQLite), not localStorage. HALF LANDED (5b784fa, merged 2026-08-20): static/chat.js (340 lines) + static/md.js (189 lines) give streaming chat with markdown and code rendering, verified against a live server. STILL OPEN, and the open half is the one this item argued for: conversations persist in LOCALSTORAGE, not server-side — there is no SQLite dependency in ferrox-server at all. A conversation tree, reasoning blocks and mid-conversation model switch are also not implemented"
    status: pending
  - id: ui-api-monitor
    content: "API Monitor screen: ring-buffered live request log. Keep duration_ms and decode_ms SEPARATE (duration carries queue wait + prefill; conflating them reads a 50 tok/s model as 5) and flag external-vs-UI traffic. BACKEND LANDED 1a59c15: GET /admin/stats serves a 200-entry ring keyed by the request_id the response already carried, so a row joins its message by equality instead of the claiming heuristic oMLX needs. duration_ms and decode_ms are separate fields all the way to the wire, and decode_ms/ttft_ms are null rather than a copy of the total when the engine did not time itself. Recorded for /v1/chat/completions (streamed and not, including rejections), /v1/completions and /v1/messages. SCREEN LANDED (5b784fa, merged 2026-08-20): static/activity.js (162 lines) renders that ring. Verified live — one streamed chat produced one ring entry carrying request_id, route, status, prompt_tokens, completion_tokens, ttft_ms, duration_ms and decode_ms as distinct fields. STILL NOT DONE: via_api_key / external-vs-UI attribution (nothing records which key served a request), a queue gauge, and /v1/embeddings, /v1/tokenize and /v1/detokenize are still unrecorded"
    status: pending
  - id: ui-embed-server
    content: "Embed the built frontend into the ferrox-server binary (rust-embed) and serve at `/` with an SPA fallback; replaces the 72-line static/ui.html stub. LANDED (4fbfe62, merged 2026-08-20): crates/ferrox-server/src/ui.rs embeds crates/ferrox-server/static/ with rust-embed and serves the shell at `/` and `/ui`, with an SPA fallback so a deep link resolves to index.html. API_PREFIXES (/v1/, /admin/, /health, /metrics, /cache/) are shielded from that fallback as PREFIXES rather than exact paths, so an unknown sibling like /v1/rerank returns 404 instead of an HTML 200 a client would misread; a test asserts the list still covers routes::ALL. The shell and its assets sit on the UNAUTHENTICATED side beside /health, because the screen a user needs in order to enter an API key cannot itself be behind that key. GATED ON FERROX_UI=1 — without it `/` is 404. VERIFIED end to end on 2026-08-20: `/` 200 text/html, the deep link `/models` 200 serving the same shell, `/app.js` 200, and a real streaming /v1/chat/completions that produced tokens and appeared in /admin/stats keyed by its request_id"
    status: completed
  - id: ui-tauri-shell
  ...
  - id: ui-settings-snippets
    content: "Settings > Connect: copy-pasteable curl / Python-SDK / IDE snippets prefilled with the live base URL, key and loaded model. Highest-leverage screen for a server whose job is being someone OpenAI endpoint. LANDED (5b784fa, merged 2026-08-20): static/connect.js fills every snippet from what is live right now — the origin the page was served from and the model id /v1/models reports — rather than from a placeholder, on the stated grounds that a snippet containing YOUR_MODEL_HERE gets pasted with YOUR_MODEL_HERE still in it"
    status: completed
