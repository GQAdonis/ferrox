...
todos:
  ...
  - id: anchor-checkpoints
    content: "THE LARGEST SINGLE OMISSION OF THE PORT, and it is pure logic with no GPU in it. FreeToken's headline 'semantic-aware caching' is NOT in ferrox-edge: only the pool-sizing term for it is (`anchor_checkpoints: bool`, crates/ferrox-edge/src/pool.rs:368), which sizes a window pool for a feature that does not exist. The mechanism has three parts, all in FreeToken's scheduler/: (1) `load_toolcall_anchor_id` -- resolve the tool-call opener for the served parser to a SINGLE token id, or None if the tokenizer spells it with more than one; `ferrox_edge::ToolCallFormat::opener` already returns that string per family, so this is an encode-and-check; (2) on each sampled token, if it is the anchor id and the request has no anchor yet and is not finishing, stamp `toolcall_anchor_len = input_ids.len()`; (3) retention: for a recurrent model, freeze the GDN state at the anchor into the idle ping-pong slot (gated on the anchor being page-aligned and equal to cached_len); for a window model, cap the window-slide threshold at `anchor - window - SWA_RETAIN_GAP`, and DROP the anchor when `threshold - cap > window + SWA_RETAIN_GAP` (decode ran too far past it) -- that unbounded-growth bound is exactly what the `anchor_checkpoints` pool term already pays for. Acceptance: an agentic turn that edits its context at a tool-call boundary reuses the prefix before the edit instead of recomputing it, shown as a cached_len in a test, plus the drop rule covered by a test that decodes far past an anchor"
    status: pending
  - id: cache-manager
  ...
  - id: swa-paged-allocator
    content: "Pure logic, not ported, and `cache-manager` needs it for any window model. The `full_to_swa_index_mapping` allocator: length `full_tokens + page_size + 1`, slot 0 reserved as the 'no live window slot' sentinel so a free is idempotent, a trailing -1 so a -1 last-loc translates to -1, allocation as a FIFO head-take off the free list, and the conservation invariant `available + count(mapping > 0) == swa_num_tokens - 1` asserted after every operation. Note the ordering hazard FreeToken hit: a rebuild must reset the mapping and the free list ATOMICALLY with the buffers. Acceptance: the conservation invariant as a test after alloc/free/double-free/exhaustion/rebuild, and exhaustion raising rather than wrapping"
    status: pending
