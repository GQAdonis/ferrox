...
todos:
  ...
  - id: paged-decode-path
    content: "NEWLY DISCOVERED GATE, found while finishing cache-manager. `wire-radix-prefix-cache` cannot be done as this plan first described it: a radix cache hands back PAGE indices, and the serving decode path does not read paged KV. `Decoder::forward_token_paged` exists and is asserted bit-identical to `forward_token` (crates/ferrox-models/src/decoder.rs:2518), but its ONLY caller is its own test -- `generate.rs` runs `forward_token` over contiguous per-layer `KvCache` buffers. Worse, the paged kernel is not a drop-in: its own comment (decoder.rs:2528) says it 'has no attention-sink term and no sliding-window arm, so running gpt-oss here would produce a different distribution than the contiguous path for the same input -- silently', and the function refuses gpt-oss with an assert rather than answer wrongly. So the work is: (1) an attention-sink term and a sliding-window arm in `causal_gqa_attention_paged`, each verified bit-identical against the contiguous kernel on the models that need them; (2) `forward_token_paged` on the serving path, behind the same greedy-id parity discipline the repo uses elsewhere; (3) only then the radix cache. This is the largest single piece of Phase A and it is a CORRECTNESS item before it is a caching one. Acceptance: `ferrox verify` greedy-id parity on the paged path for at least 3 models including one window model and gpt-oss, then the paged path serving by default"
    status: pending
  - id: wire-radix-prefix-cache
    content: "Replace `ferrox-models::prefix_cache` with `ferrox_edge::radix`. What is there today, by its own admission: 'not a trie/radix-tree structure' (crates/ferrox-models/src/prefix_cache.rs:16), `find_longest_prefix` is an O(n) linear scan over whole-conversation snapshots (:76), and `store` evicts index 0 -- FIFO, despite the module calling it LRU (:131). It also CLONES a `Vec<KvCache>` per entry, so a thousand requests off one system prompt hold a thousand copies of its KV. The radix cache shares the nodes instead and reference-counts the pages under them. Gated on cache-manager (DONE) and, as of this pass, on paged-decode-path -- the cache hands back page indices and the serving path does not read paged KV. Acceptance: two prompts sharing a 2000-token system prompt hold ONE copy of it, shown as a byte figure in `/cache/stats`, and `ferrox verify` greedy-id parity is unchanged on at least 3 models"
    status: pending
