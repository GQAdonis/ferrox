...
todos:
  ...
  - id: metal-barrier-ranges
    content: "LANDED on wf/metal-barrier-ranges (0eac360, 98be927, 4615bbc), AND IT REFUTES ITS OWN PREMISE — barriers were not the cost. Done: `moe_ranges.rs` -> `mem_ranges.rs` (`MoeMemRanges` -> `MemRanges`), and every blanket `memoryBarrierWithScope:Buffers` in attn.rs replaced by the tracker (llama `ggml_mem_ranges` / `ggml_metal_op_encode`: declare srcs+dsts per dispatch group, barrier only on a real SRC<->DST or DST<->DST conflict, narrowed to the pending resource set). 15 blanket barriers/layer gone from `encode_prefill_dense_layer`, 19 more from `launch_decode_dense_stack`, zero left in the file. Two buffers had to be pulled in for this to be SAFE and not merely fast: `encode_gqa_prefill_with_kv` / `encode_gqa_with_kv` dequantize a quantized KV cache into a shared process-wide f16 scratch that no caller can name (next layer's dequant is a WAR against this layer's GQA read), and `encode_moe_prefill_ffn` keeps its router/topk/expert scratch private and was relying on the caller's blanket barrier to order it across layers — both now take the tracker. New `MemRanges::serial()` for the Gemma sandwich decode path, whose encoder is `MTLDispatchTypeSerial`: Metal already orders those dispatches and llama likewise skips the barrier when it is not encoding concurrently; encoder kind and tracker are now chosen from one expression so they cannot drift. BOTH NAMED FUSIONS LANDED: `add_rms_norm_f32_to_f16_batch` (residual add + FFN RMSNorm + the f32->f16 staging convert in one dispatch; dense FFN only — the MoE FFN still routes on the f32 activations) and `silu_mul_f32_to_f16` / `gelu_mul_f32_to_f16` (also used inside `encode_moe_prefill_ffn`). Both are bit-identical to the pairs they replace (product still formed in f32, rounded once). The SwiGLU fusion also retires the f32 `act` plane from `PrefillScratch`, saving `batch * max_gate * 4` bytes. New tooling: `gpu::gpu_timing_note` lifts `FERROX_METAL_GPU_TIMING` out of the MoE decode stack into a tagged helper now wired to the dense decode stack and the dense prefill stack too, and `FERROX_METAL_BARRIER_LOG` joins `FERROX_METAL_MOE_BARRIER_LOG`. MEASUREMENT (PROVISIONAL; interleaved A/B against the pre-change binary in the same window, GPU-clock ms per command buffer — wall clock unusable, host load 8-34 from other agents throughout, `ferrox bench` was never run and RESULTS.md was NOT touched): Llama-3.2-1B ~900-token prefill 398.6/400.9/397.1 -> 394.7/394.1/392.3 ms (-1.3%, 3/3); Llama-3.2-1B decode 6.394/6.044/6.086/6.119 -> 6.084/6.100/6.112/6.101 ms/tok (mean 6.16 -> 6.10, inside spread); Gemma-3-1B decode, the serial path where barriers are now gone entirely, 9.486/9.612/9.573/9.622 -> 9.565/9.524/9.615/9.496 (mean 9.573 -> 9.550, i.e. a barrier on a serial encoder was already free); OLMoE decode and OLMoE prefill unchanged. WHY IT IS SMALL, three independent ways: (1) `FERROX_METAL_BARRIER_LOG` reports ~0.99 barriers per tracked op BEFORE and AFTER on both the dense and MoE decode stacks — a B=1 decode layer is a strict dependency chain (norm -> QKV -> RoPE -> KV -> attention), so a range tracker has essentially nothing to let overlap and can only narrow each barrier's scope; (2) narrowing scope is worth ~1%; (3) the arithmetic agrees — Llama-3.2-1B Q4_K_M is ~0.8 GB and 6.1 ms/tok is ~130 GB/s against the M2 Pro's 200 GB/s, so decode is already weight-bandwidth-bound. CORRECTION to `metal-moe-decode`'s closing sentence: the ~2.4 ms/tok attention at ~63 GB/s is NOT explained by `~8 barrier points per layer`. Whatever is left on the OLMoE tg128 row is inside the attention kernel or the host lm_head, and the next attempt should sample or GPU-capture that kernel rather than touch synchronisation again. CORRECTNESS: `ferrox verify --backend metal` identical cpu-vs-metal ids with PREFILL COVERED in 54 runs, all green, deliberately repeated because a missing barrier is a race that passes most runs — 24 at `--prompt-tokens 64` (4 each on Llama-3.2-1B dense, OLMoE MoE, Gemma-3-1B sliding-window, Gemma-2-2B sandwich/serial-encoder, Qwen3-0.6B QK-norm, SmolLM2-135M small-hidden), 18 more at 64 after the last change, and 18 across prompt lengths 100/300/512. `cargo test -p ferrox-metal --features metal -- --ignored`: 46 passed. All six gates green (workspace build, metal build, workspace test, both clippy invocations, fmt --check). OWED: nothing landed here is published — no `bench --suite` run was taken (host load never came near the 2.0 bar; it sat at 5-130 all session) and benchmarks/RESULTS.md is untouched, so every number above is PROVISIONAL and the ledger still shows the old rows."
    status: completed
  - id: metal-mm-occupancy
    content: "LANDED on wf/metal-barrier-ranges (fcf119c). `mul_mm_sg_impl` and `mul_mm_sg_impl_f16` are templated on a `BC_OUT` flag and each quant type now emits a second `_a` entry point with the ragged-edge staging arm compiled out. That staging arm — a 64x32 f32 threadgroup buffer for the partial tile — is the only reason the kernel asked for 8192 bytes; the GEMM's own sa/sb tiles need 4096 + 2048 = 6144, which on a 32 KiB per-core budget is 5 resident threadgroups instead of 4. `mul_mm_sg_variant` picks the `_a` pipeline and the smaller allocation whenever the dst tile grid is exact (`rows % 64 == 0 && batch % 32 == 0`), which is llama's `bc_out` condition and llama's two sizes (`ggml-metal-device.cpp`: `res.smem = bc_out ? 8192 : (4096 + 2048)`). The `_a` pipelines are warmed next to their siblings, and the standalone `launch_*_mul_mm_sg` entry points route through the same picker. PROVISIONAL A/B, interleaved against the immediately preceding commit's binary, 8 reps over two windows at host load 6-25, on a 704-token prompt (704 = 22*32, chosen so the exact-tile pipeline is actually taken): whole-prefill GPU-clock mean 397.2 -> 391.9 ms, -1.35%, 6 of 8 reps favouring it. SCOPE CAVEAT worth recording: `batch` is the whole prompt length (prefill is not chunked), so on real traffic the exact-tile pipeline fires about 1 prompt in 32. Padding the batch to a multiple of 32 would make it fire always and is the obvious follow-up, but it changes what the GEMM computes and was not attempted here. Coverage: `mul_mm_sg_variant_picks_the_small_threadgroup_only_on_exact_tiles` pins the selection rule without a GPU (choosing `_a` for a ragged shape would write past the matrix, so that rule is the load-bearing part), and the seven `*_matches_the_matvec_it_replaces` hardware tests gained a `(128, 512, 64)` shape — an exact grid with more than one tile in both dims, where a wrong epilogue guard shows. 36 `ferrox verify` runs green with prefill covered, including prompt lengths 100 and 300 (ragged, must fall back) and 512 (exact). OWED: same as metal-barrier-ranges — no suite run, RESULTS.md untouched, number is PROVISIONAL."
    status: completed
  ...
  - id: tooling-bench-discipline
    content: "ADOPT into ferrox bench: warmup before any timing (shader/JIT compilation), temp=0 on timed runs, assert prompt length before AND after generation, assert zero cache hits, record thermal pressure with each result. ferrox is already AHEAD on repeat-and-median and on having a checked-in ledger at all. HALF LANDED (afdf2de, 2010be8): the load bar is now ENFORCED rather than merely written down — ferrox-cli/src/host_state.rs reads the 1-minute load average and a timed run REFUSES above DEFAULT_MAX_LOAD = 2.0, with both the start and end readings written into the receipt so a published row can be audited later. `None` is kept distinct from `0.0` on purpose: `the host was idle` and `we could not tell` are different facts. A second gate refuses to be the second ferrox process holding the same model, which is how several of this plan A/B windows were silently ruined. STILL OPEN: warmup before timing, the temp=0 assertion, the prompt-length assertions, the zero-cache-hit assertion, and thermal pressure in the receipt"
    status: pending
  ...
  - id: coverage-cheap-archs
    content: "PARTLY LANDED, and the fixture built for it found a bigger bug than the item (branch wf/coverage-stop-bos). CORRECTION to this item own text: the tensor is `blk.N.exp_probs_b.bias`, NOT `ffn_exp_probs_b` — llama-arch.cpp:416 and gguf-py/gguf/constants.py:1240. LANDED AND VALIDATED: (1) exp_probs_b in the generic MoE loader, applied as build_moe_ffn does — added to the SELECTION score only, combine weight from the unbiased score — plus `expert_weights_scale` (was ignored outright) and `expert_weights_norm` (was guessed from the arch name even when the file said so). (2) ROPE LAYOUT, which is the real find: running the gpt-oss `wrong RoPE list` comparison over the whole table against `llama_model_rope_type` finds 24 MORE archs on the generic GQA path that ferrox rotated as interleaved when llama.cpp says NEOX — afmoe, apertus, bailingmoe2, codeshell, dots1, exaone, exaone-moe, grovemoe, hunyuan-dense, hunyuan-moe, laguna, mellum, mimo2, minicpm3, nemotron, openelm, orion, plamo, plamo3, seed_oss, smallthinker, starcoder2, step35, talkie. Each loaded, ran at full speed and rotated the wrong pairs of every Q/K head. `tests/rope_layout.rs` now pins the table against a transcription of the reference switch. PROOF: `scripts/make_dots1_fixture.py` + `tests/moe_routing_bias.rs` compare prefill/decode/multi-seq against golden logits from llama.cpp own dots1 implementation on the same synthetic file; worst case 1e-6, and it was the RoPE fix that took it from 1.1e-2 to there. Mutation-checked (drop bias / drop scale / drop renormalisation / bias the weight too). (3) A second name bug found on the way: mla_gguf_loader and glm52_gguf_loader also asked for `ffn_exp_probs_b.bias`, and their synthetic fixtures wrote the same wrong name so the tests agreed with the bug. GLM failed loudly (required tensor); MLA was SILENT (optional tensor), so a real DeepSeek-V3 checkpoint routed with its bias dropped. Fixed, mutation-checked, but validated only against llama.cpp tensor-name table — no DeepSeek/GLM checkpoint is downloaded here. REFUSED RATHER THAN FAKED, and deliberately: granite/minicpm multipliers (`logit_scale`, `residual_scale`, `embedding_scale`, `attention.scale`) are refused by name when non-no-op, because residual_scale multiplies both branch outputs before every residual add — every CPU path plus the fused Metal kernels that fold the residual in — and half of that is exactly the silent divergence the gate exists to stop. minicpm is refused outright: llama.cpp hardcodes its three multipliers even when the GGUF carries no key, so a key-presence gate cannot see it. cohere2 parallel residual is likewise a REFUSAL, extended to every arch with that topology after reading each one: command-r, cohere2, cohere2moe, falcon, gptneox, phi2, plamo. STILL OPEN: the actual math for granite/minicpm multipliers and for parallel residual; `exp_probs_b` combined with expert groups (refused at load — llama.cpp masks biased scores per group before a global top-k, which is not ferrox per-group top-k); GPU — the Metal MoE router kernels take no bias input and no scale uniform, so a layer carrying either now keeps its model off the GPU MoE fast paths rather than routing without them, i.e. correct but slower; UNVALIDATED on any published dots1/ernie4_5-moe/bailingmoe2/exaone-moe/hunyuan-moe/afmoe checkpoint — none downloaded, so the claim is `matches llama.cpp on a synthetic dots1 carrying the feature`, not `runs dots1`. `partial rotary + full bias` was not touched: both are tensors, so the tensor-consumption gate already refuses them loudly rather than computing them wrong"
    status: pending
  - id: hygiene-clippy
    ...
    status: completed
  - id: legacy-cleanup
    content: "AUDITED; one item deleted, three must NOT be. DELETED (wf/coverage-stop-bos): the host-side mul_mm_id gather/scatter the GPU map0 kernel replaced when metal-moe-stack landed (ee35372) — `encode_moe_gather_rows`, `encode_moe_scatter_rows`, their two MSL kernels, and the `contig_in`/`contig_out` scratch buffers that existed only to feed them. All were #[allow(dead_code)] with zero callers. NOT DEAD, checked before touching: (a) `pack_q8_k_qs_x4_i8` is already gone from production — the only surviving copy is `reference_q8_kx4_block_qs` in repack.rs tests, which is the independent golden the replacement `prepare_q8_k_acts_x4` is checked against, and deleting it would remove the only check on the new kernel; keep it, per this plan own `Keep: one scalar/non-SIMD CPU reference for golden tests`. (b) the `by_row` transposes are already gone — zero hits in the workspace. (c) the CUDA per-position batch arm (weight_matrix.rs) is LIVE and its replacement has not landed: its own comment records that without it a batched prefill falls through to the CPU branch and never touches the GPU (SmolLM2 pp512 measured at 28 tok/s on an RTX 4090). CUDA `mul_mm` is out of scope this push, so deleting this would be a regression, not a cleanup. (d) the HOST `moe_mm_id_map0` also stays — `launch_moe_prefill_q4_0` still calls it on the host-routed path a layer falls back to when the fused stack does not apply"
    status: completed
