...
todos:
  ...
  - id: coverage-phi-metal-rope
    content: "LANDED (branch wf/phi-metal-rope). The four Metal RoPE kernels (rope_interleaved_heads / rope_neox_heads and their _batch twins) now take two uniforms ported from ggml's kernel_rope_norm / kernel_rope_neox: rot_dim (ggml n_dims) and mscale. Three things had to move together, not one: n_dims scopes the loop, it scopes the FREQUENCY EXPONENT (pow(freq_base, -i0/n_dims), not /head_dim), and for NeoX it scopes the split-half pairing — so a wrong width re-pairs channels rather than merely rotating too many. Channels [n_rot, head_dim) are copied through untouched. THE WARNING WAS HEEDED: mscale is folded into cos/sin exactly as rope_yarn does, so it cannot reach the pass-through tail; the hardware tests assert that tail comes back BIT-IDENTICAL (assert_eq, not a tolerance), in both layouts, single-token and batch. NOT AttnExtras in the end: the three values are one ggml_rope_ext call and AttnExtras is per-layer and absent from half the launch entry points, so they ride a new Copy `MetalRope { layout, rot_dim, attn_factor }` that replaces the bare MetalRopeLayout parameter already threaded through all 14 entry points — they travel together instead of one being forgotten at a call site. Decoder::metal_rope fills it from config.rope_dim / config.rope_attn_factor, the same two fields the CPU path reads. layer_supports_metal_attn no longer refuses on either; it still refuses an ODD n_rot (ggml_rope_impl asserts even). The one path that already applied attn_factor host-side before handing q/k to Metal (launch_prefill_attn_block / _o_residual) passes .attn_factor_applied_by_caller() — folding it in twice would square it. SECOND BUG FOUND BY LETTING PHI ONTO THE PATH: eight launch entry points asserted ff.len() == head_dim/2, but ggml sizes rope_freqs by the ROTARY width; Phi ships 48 entries against a 128-wide head and tripped 48-vs-64 in launch_prefill_dense_stack. Folded into one assert_freq_factors_len. THIRD, AND IT INVALIDATED THE INSTRUMENT: `verify --backend metal` set FERROX_METAL=1 and nothing else, which turns on the Metal MATVECS only — the fused attention block, its RoPE kernels and the resident KV are behind FERROX_METAL_ATTN, which `run --ngl 99` sets and verify did not. So every previous `verify --backend metal` verdict in this plan was taken on a graph nobody runs. verify now defaults it on for the accelerated child (ablation still possible by pre-setting it). Re-checked with the fused path live: TinyLlama, Llama-3.2-1B Q4_K_M, Llama-3.2-3B, Qwen3-0.6B, Qwen2.5-0.5B/1.5B, SmolLM2-135M, Gemma-2-2B, Gemma-3-1B, OLMoE, Phi-3-mini and DeepSeek-R1-Distill-1.5B all still identical cpu-vs-metal at 64 tokens with prefill covered — no regression, and no model was relying on the hole. (Gemma-4 needs its own engine and this harness cannot load it, unchanged.) VALIDATION. verify --backend metal identical cpu-vs-metal ids, prefill covered, at 64 / 128 / 300 / 1024 / 2048 AND 4200 tokens — the 4200 crosses rope.scaling.original_context_length (4096), so the LongRoPE long factor set is exercised at positions past the boundary, which is the branch a short test never reaches. (verify does not call apply_runtime_context, so it always picks the long set for this checkpoint — the short set is what a `run -c 4096` picks, and at kernel level it is the same code path with a different 48-entry buffer.) `ferrox parity` against llama.cpp's own libllama, 32-token prompt: ferrox CPU KL(llama||ferrox) 2.358e-4 / top-10 9/10, ferrox METAL with the fused attention live 2.353e-4 / 9/10 — Metal reproduces the CPU graph, which is the one already validated against llama. MUTATION-CHECKED FOUR WAYS, all restored: forcing rot = head_dim in the kernel fails rope_partial_rotary_matches_cpu, rope_mscale_scales_only_the_rotated_channels and rope_phi_long_context_factors_match_cpu_past_orig_ctx, AND makes `verify` DIVERGE at token 0 on Phi-4-mini (metal=[0,0,0]) — proof the kernel is on the model's hot path and not merely unit-tested; dropping `* mscale` moves parity to KL 1.962e-4 / top-10 7/10, i.e. a different distribution, while a hardcoded mscale=5.0 diverges outright (that pair is what proves the uniform is bound and live, since the greedy-id probe alone is insensitive to a 1.19x magnitude scale); and restoring the old `rope_dim.is_some() || rope_attn_factor != 1.0` refusal fails the new decoder gate test. OWED, AND NOT DONE HERE ON PURPOSE: benchmarks/RESULTS.md still carries Phi-4-mini metal pp512 541.43 (1.04x) and tg128 51.29 (0.98x) from BEFORE the model was refused — taken on the wrong graph, KNOWN-INVALID, and not to be quoted. This host sat at load 5.9-20.6 against `ferrox bench`'s enforced 2.0 bar for the whole session, so no timed run was attempted and the bar was not overridden. The re-measurement belongs to the maintainer's quiet-host suite run: `bench --suite --id phi4_mini_q4km --backend metal`. benchmarks/suite.json needed no change (the entry already lists both backends). docs/MODELS.md's Metal decode cell went from '— (refused, see above)' to '— (owed, see above)' and docs/FEATURES.md no longer says CPU-only; neither states a number."
    status: completed
