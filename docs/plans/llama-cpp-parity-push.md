...
todos:
  ...
  - id: coverage-jinja-templates
    content: "HALF LANDED (merged). ferrox-models::chat_template now evaluates the GGUF's own tokenizer.chat_template with minijinja instead of sniffing a model into one of six hand-written variants, and four real templates lifted from actual checkpoints are pinned in tests with exact expected output. STILL OPEN, and it is the half that matters for users: NEITHER THE CLI NOR THE SERVER CALLS IT. The serving path still sniffs, so every family outside the six still falls back to Plain in practice and tool-calling formats are still unreachable. Switching the call sites is the remaining work. Consequence to handle when it lands, recorded by the BOS sweep: many real templates emit `{{ bos_token }}` themselves and Unsloth strips it from the ones it bakes in, so whether a loader double-adds BOS becomes a property of the specific checkpoint rather than of the code. tests/bos_policy.rs already measures this across 26 local checkpoints"
    status: pending
