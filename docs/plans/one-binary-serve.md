...
todos:
  ...
  - id: server-shim-crate
    content: "HALF LANDED. The crate SHAPE is done: `ferrox-server` is still a published binary crate, its `[[bin]]` is now a four-line shim over `ferrox_server::run_server`, `cargo install ferrox-server` produces the same executable with the same flags, and the smoke test above ran the standalone binary and `ferrox serve` side by side to prove it. OPEN HALF: nothing was said about deprecation anywhere -- the crate description is unchanged and no policy about 'keep publishing for at least two minor versions' is recorded in the manifest or the README. That is deliberate: whether ferrox-server is deprecated at all depends on release-and-install-path, which is still undecided, and announcing a deprecation before deciding what the tarball ships would be the wrong order"
    status: pending
  ...
  - id: docs-single-binary
    content: "Rewrite the install and usage surface once the shape is decided: README, docs/CLI.md (the flag list above), docs/API.md, docs/AGENTS_COOKBOOK.md, docs/CONFIG.md, crates/ferrox-cli/README.md, crates/ferrox-server/README.md. State the feature explicitly, a reader who runs `cargo install ferrox-cli` and finds no `serve` subcommand has hit exactly the confusion this plan exists to remove, and the error for the missing subcommand should say `rebuild with --features serve`, not `unrecognized subcommand`. STATUS 2026-08-24: untouched on purpose, per this plan's own 'order of work' (docs last, once the shape stops moving). One part of it is already done in code: the missing-subcommand error is now a sentence naming `--features serve`, not clap's 'unrecognized subcommand'. Everything else is still true and still owed -- as of this branch `ferrox serve` EXISTS and NO document mentions it, so docs/CLI.md's subcommand list is now incomplete as well as its flag list"
    status: pending
  - id: hygiene-cross-target-gate
    content: "ADDED 2026-08-24 after it broke two releases in one week. CI tracks `stable` and a developer checkout drifts behind it, so a stable bump turns a green branch red at TAG time with lints nobody ran: 1.97 brought manual_checked_ops and explicit_counter_loop, 1.98 rejected chunks_exact with a compile-time-known size (58 errors). Worse, the 1.98 fix passed every local gate and STILL broke CI, because the sites it got wrong live in the scalar fallbacks compiled only when the NEON paths are not: on an Apple-silicon laptop `cargo build --workspace` never sees them. Two things to land: (1) `cargo check`/`clippy --target x86_64-apple-darwin` in front of every push, which compiles those fallbacks locally and is the check that was missing, and (2) a rust-toolchain.toml pin so CI and every developer share a compiler and lint set, bumped deliberately in its own commit with the resulting fixes. The pin was attempted and BACKED OUT: rustup fought the cargo processes several agents were holding and left two toolchains half-installed, so it needs a quiet machine"
    status: pending
