...
todos:
  ...
  - id: serve-subcommand-parity
    content: "`ferrox serve` must accept exactly what `ferrox-server` accepts today, or it is a downgrade dressed as a simplification. Take the list from the ServerArgs struct, NOT from docs/CLI.md, which documents eight flags and reads as exhaustive: -m/--model, --host, --port (including 0 for a kernel-assigned port), -t/--threads, -dev/--device, -ngl/--n-gpu-layers, --exit-on-stdin-close, --list-devices, --mcp-config, --allow-multiple-instances. CORRECTION since this was written: --ui-server and FERROX_UI NO LONGER EXIST. ferrox-server stopped serving the UI in v0.9.0 and Ferrox Studio is a standalone app under ui/, so do not resurrect them. Carry the `ferrox.server.ready` stdout line unchanged, since supervisors parse it"
    status: pending
