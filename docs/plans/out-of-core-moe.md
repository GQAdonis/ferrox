...
| **Budget** | Bytes in, **expert slot count** out. `ds4_ssd.c:74` is `bytes / per_expert_bytes`. `ferrox_core::expert_budget::plan_cache_budget` (`pool.rs:170`) already computes it, with ds4's two-layer prefill headroom (`pool.rs:158`) arrived at independently. Nothing calls it. |
...
`ferrox-core/src/residency.rs:191` and `ferrox-moe/src/lib.rs`;
`RebuildRejected` in both `expert_cache.rs:1150` and `pool.rs:231`.

| | `ferrox-core::ExpertStore` | `ferrox-core::ExpertCache` + `expert_slots` |
|---|---|---|
...
then  metal routing readback    cheap, wrong independent of this plan
      ...
      prefill double buffer     already built in ferrox-core, unwired
