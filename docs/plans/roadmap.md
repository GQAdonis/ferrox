...
todos:
  ...
  - id: a0-dissolve-ferrox-edge
    content: "TOP PRIORITY, ahead of everything else, because it is finishing work already paid for rather than starting work. `ferrox-edge` is 28,384 lines. It was meant to be FreeToken's host-side policy INTEGRATED into the ferrox stack, and it became a separate crate that is roughly half wired. MEASURED 2026-08-27, external references per module: `cache_manager` 2000 lines / 0 refs, `state_pool` 806 / 0, `supervisor` 746 / 0, `stats` 576 / 0, `maintenance` 499 / 0, `window_pool` 396 / 0, `detokenize` 379 / 0. That is about 5,400 lines nothing outside the crate calls, and much of the remainder is reachable only through a single re-export. NOT A SIMPLE DELETE: those modules call EACH OTHER, `detokenize` alone has 12 internal references, so the dead set has to be cut as a graph rather than a list. THE PRIZE IS NOT THE LINE COUNT, it is that `freetoken-parity` reached 60 of 73 todos before being archived, so most of this is finished engineering sitting one integration away from being usable. IT ALSO RESOLVES A CORRECTNESS HAZARD: there are TWO independent expert caches that do not know each other exists, `ferrox_core::expert_store` (608 lines, wired into both decode paths, proven bit-identical at a 1-byte budget) and `ferrox_edge::expert_cache` plus `expert_slots` (3,285 lines, 50 tests, zero production callers). Each owns its own byte budget, and on unified memory that is the same physical RAM counted twice. DONE MEANS: dead modules deleted, live ones moved into the crate that actually uses them (parsers/effort/stats to ferrox-server, qstar and bench_* to ferrox-cli, the `SlotDevice` seam to ferrox-core beside `expert_store`), the crate gone, and `expert_store` the single budget holder. ATTRIBUTION STAYS: FreeToken is Apache-2.0 and its notice in docs/THIRD_PARTY_NOTICES.md must survive in proportion to the code that survives, which is a licence obligation and not a preference. The crate name and in-code references can go; the notice cannot."
    status: pending
...
| # | Theme | Ships what |
|---|---|---|
| **1** | Dissolve `ferrox-edge` into the stack | ~5,400 dead lines gone, 60/73 finished todos become usable, one expert cache instead of two |
| **2** | Fix what is wrong, and close the oracle's hole | Correct output on models that run today. Small, immediate |
| **3** | Model layer: audit, vocabulary, then split | A correctness fix per audit finding, then one architecture at a time |
| **4** | Close the 68 by inverting the default | Honest refusals instead of silent wrong answers |
| **5** | Out-of-core MoE, and one large real checkpoint | A model that did not fit, running. First milestone is "does it load" |
| **6** | CPU decode scaling | One item carrying most of the remaining CPU gap |
| **7** | Hardware reach: backend seam, then Vulkan | Every AMD and Intel GPU, currently none |
| **8** | The rest: embeddings, quants, serving, measurement | Breadth, once the above hold |
