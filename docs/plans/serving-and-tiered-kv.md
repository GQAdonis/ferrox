...
todos:
  ...
  - id: mem-preload-kv-budget
    content: "NOT TOUCHED THIS ROUND, and mostly already landed elsewhere -- recorded here so the next run does not duplicate it. `ferrox-models::kv_budget` already has the whole inequality (`KvBudget::check`, `estimated_bytes`, `kv_bytes_available`, `max_context`, `Ceiling`, `KvBudgetError`) with the sliding-window and MLA variants the plan asks for, and `ferrox-cli`'s run path already prices it BEFORE the load and refuses with `Ceiling::DeviceMemory` rather than OOMing (`crates/ferrox-cli/src/run.rs`, `--ctx-size auto`). THE REMAINING HALF is ferrox-server: it loads `FERROX_MODEL_PATH` without pricing the model at all, so the scheduler's ceilings have to be configured by hand (`FERROX_CB_KV_BLOCKS`, `FERROX_CB_MAX_CONTEXT`) instead of derived from weights + per-token KV + headroom against the device budget. Left pending deliberately: doing it properly means threading a priced `KvBudget` from the loader through `Active` into the request path, which is a change to the model-loading path rather than to the scheduler, and this run's remit was the scheduler."
    status: pending
  - id: mem-ctx-auto
  ...
  - id: mem-typed-rejection
    content: "HALF LANDED. DONE: the batched request path now refuses against two immovable ceilings with a typed 400 rather than running until something OOMs. `DecodeError::KvBudgetExceeded` carries `binding` (a `ferrox_models::Ceiling` code -- `context_length_exceeded` for `FERROX_CB_MAX_CONTEXT`, `device_memory_budget_exceeded` for the whole `FERROX_CB_KV_BLOCKS` budget), `estimated_bytes`, `limit_bytes`, `positions`, `positions_limit` and a detail string; the bytes are priced from the model's real `KvShape` (sliding-window cap included), not a restatement of the block count, so an operator can check them against `inspect-plan`. The context ceiling is reported FIRST when both bind, because it is the one a client can act on. `retry_after_secs()` is `None` for both and the status is 400, since an idle server refuses them identically. Counters split three ways -- `kv_rejected_context_length`, `kv_rejected_too_large`, `queue_rejected` -- all on `/metrics`, and the 400 body carries the same fields for clients that never read headers. Tests confirmed to FAIL when the ceiling check is bypassed: `a_request_longer_than_the_context_ceiling_names_that_ceiling`, `the_context_ceiling_is_reported_before_the_device_ceiling`, `a_request_larger_than_the_whole_budget_is_refused_rather_than_queued`. THE OPEN HALF: this covers ONLY the continuous-batching path. The private `generate` path still answers 503 `KvPoolExhausted` and has no context ceiling of its own, so a long prompt there is still charged before anything notices. And the ceilings are configured (`FERROX_CB_MAX_CONTEXT`, `FERROX_CB_KV_BLOCKS`) rather than DERIVED from the model plus the device budget -- `ferrox-models::kv_budget` already computes exactly that for the CLI (`KvBudget::check`, `max_context`), but ferrox-server does not price its model at load time, so nothing joins the two. That join is `mem-preload-kv-budget`'s remaining server half."
    status: pending
isProject: false
---
